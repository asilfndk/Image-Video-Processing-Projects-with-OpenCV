...
def checkParkSpace(imgg):
    ...
    for pos in posList:
        x, y = pos
        imgg_crop = imgg[y:y + height, x:x + width]
        count = cv2.countNonZero(imgg_crop)    
        print("count: ", count)
        ...
        cv2.putText(img, str(count), (x, y + height - 2), cv2.FONT_HERSHEY_PLAIN, 1, 1)
...
with open("parking_space_pos", "rb") as f:
